...
/**
    ...
    @created 2004/08/20, rewritten 20080804
*/
public class HubMerger {
    ...
    */
    ...
    private void init(Hub hubRoot, Hub hubCombinedObjects, String propertyPath, boolean bShareActiveObject, String selectOrder, boolean bUseAll) {
      //System.out.println("===> "+hubRoot+",  path => "+propertyPath);       
        ...
        dataRoot = new Data(nodeRoot, null, hubRoot);
...
//qqqqqqqqqqqq todo: Recursive functionality            
//OALinkInfo recursiveLinkInfo = OAObjectInfoDelegate.getRecursiveLinkInfo(oi, OALinkInfo.MANY);
            ...
            if (bShareEndHub) {
            	if (linkInfo.getType() == OALinkInfo.MANY) {
					if (st.hasMoreTokens()) bShareEndHub = false;  // only the last one can be many
            	}
            	else {
					if (!st.hasMoreTokens()) bShareEndHub = false;  // only the last one can be many, but not one
            	}
            }
    ...
    public void close() {
        ...
        dataRoot = null;
        // test
        // System.out.println("HubMerger.close TotalHubListeners="+TotalHubListeners);        
    }
    ...
    class Data extends HubListenerAdapter {
        ...
        Data(Node node, OAObject parentObject, Hub hub) {
        	...
        	if (!node.clazz.equals(hub.getObjectClass())) throw new RuntimeException("Hub class does not equal Node class");
        	this.node = node;
...
if (true) return;            
        ...
        void createChildren() {
        	...
        	if (node.child == null) {
                ...
                // was: 20101127 
                // if (bUseAll || (this != dataRoot && dataRoot != null)) {
                if (bUseAll || this.node != nodeRoot && nodeRoot != null) {
                    ...
                    if (obj != null) createChild(obj);
                        ...
                        // 20110809 see if hub.masterObject can be used
        ...
        private boolean _isUsed(Object objFind, Node nodeFind) {
        	...
        	if (!bEnabled) return false;
        	if (node.child == null) {
        		return (nodeFind == null && hub != null && hub.contains(objFind));
        	}

    		if (this.node.child == nodeFind) {
        ...
        void remove(Object obj) {
        	if (!bEnabled) return;
			////XOG.finer("remove "+obj.getClass().getName());
        	
            if (alChildren == null) {
                if (!bShareEndHub && !isUsed(obj)) {
                    if (this.hub == hubCombined) {
                        if (!hubCombined.contains(obj)) return;  // might have already been removed
                    }
                    if (OAThreadLocalDelegate.isHubMergerChanging()) { // 20120102
                        HubAddRemoveDelegate.remove(hubCombined, obj, false, false, false, false, false);
                    }
                    else {
                        if (bServerSideOnly) OAClientDelegate.serverSideCode(); 
        	...
        	for (int i=0; ; i++) {
                ...
                try {
                    lock.readLock().lock();
                    if (alChildren == null || i >= alChildren.size()) break;
                    child = alChildren.get(i);
                }
                finally {
                    lock.readLock().unlock();
                }
				if (obj == child.parentObject) {  // will always be a type=Many
	                try {
	                    lock.writeLock().lock();
	                    if (alChildren != null) {  // could have been closed 
	                        this.alChildren.remove(i);
	                    }
	                }
	                finally {
	                    lock.writeLock().unlock();
...
//System.out.println("onNewList ===> "+hubRoot+",  => "+path);     
