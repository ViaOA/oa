...
public class ApplicationStatusDelegate {
    ...
    public static ApplicationStatus getApplicationStatus(com.theice.tsac.mrad.model.ApplicationStatus clientAppStatus) {
        int type = ApplicationStatus.TYPE_UNKNOWN;
        if (clientAppStatus != null) {
            if (clientAppStatus == com.theice.tsac.mrad.model.ApplicationStatus.Started) {
                type = ApplicationStatus.TYPE_STARTED;
            }
            else if (clientAppStatus == com.theice.tsac.mrad.model.ApplicationStatus.Waiting) {
                type = ApplicationStatus.TYPE_WAITING;
            }
            else if (clientAppStatus == com.theice.tsac.mrad.model.ApplicationStatus.Running) {
                type = ApplicationStatus.TYPE_RUNNING;
            }
            else if (clientAppStatus == com.theice.tsac.mrad.model.ApplicationStatus.Paused) {
                type = ApplicationStatus.TYPE_SUSPENDED;
            }
            else if (clientAppStatus == com.theice.tsac.mrad.model.ApplicationStatus.Stopped) {
