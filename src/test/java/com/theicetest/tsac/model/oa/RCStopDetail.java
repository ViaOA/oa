...
@OATable(
    indexes = {
        @OAIndex(name = "RCStopDetailRcStop", columns = { @OAIndexColumn(name = "RcStopId") })
    }
)
public class RCStopDetail extends OAObject {
    ...
    @OAOne(
        ...
        reverseName = RCDeployDetail.P_RCStopDetail
    )
    ...
    @OAOne(
        displayName = "RC Stop", 
        reverseName = RCStop.P_RCStopDetails, 
        required = true, 
        allowCreateNew = false
    )
    @OAFkey(columns = {"RcStopId"})
    ...
    public void load(ResultSet rs, int id) throws SQLException {
        ...
        if (rs.wasNull()) {
            OAObjectInfoDelegate.setPrimitiveNull(this, RCStopDetail.P_TotalTimeMs, true);
        }
        ...
        int rcStopFkey = rs.getInt(10);
        if (!rs.wasNull() && rcStopFkey > 0) {
            setProperty(P_RCStop, new OAObjectKey(rcStopFkey));
        }
